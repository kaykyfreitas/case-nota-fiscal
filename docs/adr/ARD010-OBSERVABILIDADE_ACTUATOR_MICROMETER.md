...
- Micrometer no `core`: `NotaFiscalMetrics` e `MdcPropagacao` em `core.observability` (o gerador não importa `adapter`). `http.server.requests` automático; **Counter `nf.emitida`** só depois das integrações com sucesso; timers `nf.gerar` (depois da conferência até o join) e `nf.integracao` (`integracao=estoque|registro|entrega|financeiro`).
